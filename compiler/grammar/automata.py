...
from compiler.grammar.item import Item
...
class Automata:
    ...
    def __init__(self, filename: str) -> None:
        ...
        self.follow: dict[str, set[str]] = {
            nt: set() for nt in self.reader.non_terminals
        }
    ...
    @staticmethod
    def _state_to_key(items: list[Item]) -> str:
        return "|".join(sorted([str(item) for item in items]))

    def _closure(self, items: list[Item]) -> list[Item]:
    ...
    def _goto(self, items: list[Item], symbol: str) -> list[Item]:
        """Переход по символу"""
        new_set = []
        for item in items:
            if item.next_symbol == symbol:
                new_set.append(Item(item.rule, item.dot_pos + 1))
        return self._closure(new_set)

    def build_lr0_states(self) -> None:
        """Построение lr-0 состояний"""
        start_rule = self.reader.rules[0]
        initial_state = self._closure([Item(start_rule, 0)])

        self.states = [initial_state]
        state_keys = {self._state_to_key(initial_state): 0}
        ...
        while queue:
            ...
            for symbol in symbols:
                next_state = self._goto(curr_state, symbol)
                if not next_state:
                    continue
