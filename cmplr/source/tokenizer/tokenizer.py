...
class Tokenizer:
    ...
    def parse_code(self):
        while self.parse_index < len(self.text):
            ...
            if self.parse_index < len(self.text):
                res = self.parse_symbol() or \
                      self.parse_string_constant() or \
    ...
    def parse_symbol(self):
        ...
        if cur_symb in self.symbols:
            self.tokens.append(Token(TokenType.symbol, cur_symb, self.cur_row, self.cur_col))
            self.parse_index += 1
            self.cur_col += 1
            return True
        return False
    ...
    def parse_integer_constant(self):
        if self.text[self.parse_index].isdigit():
            ...
            sb.append(self.text[self.parse_index])
            self.parse_index += 1
            while self.parse_index < len(self.text) and self.text[self.parse_index].isdigit():
                sb.append(self.text[self.parse_index])
                self.parse_index += 1
            num_str = ''.join(sb)
            try:
                ...
                if result <= 32767:  # TODO: for JackE should be large
                    self.tokens.append(Token(TokenType.integerConstant, num_str, self.cur_row, self.cur_col))
                    self.cur_col += len(sb)
                    return True
                else:
    ...
    def parse_identifier_and_keywords(self):
        ...
        if (cur_symb.isalpha() and 'A' <= cur_symb <= 'z') or cur_symb == '_':
            sb = []
            while True:
                if not ((cur_symb.isalpha() and 'A' <= cur_symb <= 'z') or cur_symb.isdigit() or cur_symb == '_'):
                    break
                sb.append(self.text[self.parse_index])
                self.parse_index += 1
                if self.parse_index >= len(self.text):
            ...
            if s.lower() in self.keywords:
                self.tokens.append(Token(TokenType.keyword, s, self.cur_row, self.cur_col))
            else:
                self.tokens.append(Token(TokenType.identifier, s, self.cur_row, self.cur_col))
            self.cur_col += len(sb)
            return True
        return False
